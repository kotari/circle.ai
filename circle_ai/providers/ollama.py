...
class OllamaWrapper:
    ...
    def _validate_ollama_endpoint(self, endpoint: str) -> None:
        if endpoint not in OLLAMA_ENDPOINTS:
            raise NotImplementedError(f"ollama endpoint must be one of `{OLLAMA_ENDPOINTS}")
        
    
    def _call_generate_endpoint(self, model: str="llama2", prompt: str="why is the sky blue?", stream: bool=False, **kwargs,):
        resp =  self.ollama_client.generate(model,prompt,stream=stream,**kwargs)
        return resp
    

    def send_ollama_request(self, endpoint: str, model: Optional[str]= None, prompt: Optional[str]= None, stream: bool=False, **kwargs):
